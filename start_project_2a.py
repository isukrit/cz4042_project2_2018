...
def load_data(file):
    ...
    Y[np.arange(labels.shape[0]), labels-1] = 1

    X = (data - np.min(data, axis = 0))/np.max(data, axis = 0)
...
def main():
    ...
    trainX, trainY = load_data('./cifar-10-batches-py/data_batch_1')
    testX, testY = load_data('./cifar-10-batches-py/test_batch_trim')
    # Create the model
